...
const computeDocumentPath = (slug: string) => {
  return Settings.gitload
    ? `${GitHubLink.href}/raw/main/contents/blog/${slug}/index.mdx`
    : path.join(process.cwd(), "/contents/blog/", `${slug}/index.mdx`)
}
...
export async function getTable(
  ...
  if (Settings.gitload) {
    const contentPath = `${GitHubLink.href}/raw/main/contents/blog/${slug}/index.mdx`
    try {
      const response = await fetch(contentPath)
      if (!response.ok) {
        throw new Error(
